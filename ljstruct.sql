...
AUTO_INCREMENT=2146 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci';
...
AUTO_INCREMENT=55260 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci'
...
/* insert into inventory (kodebrg,qty,satuan,faktur,typetrans,idTrans,tglTrans,kodeGudang,IDuser,username,waktu,keterangan) 
 values (new.kode,new.UT,new.satuan,'STOK AWAL','STOK AWAL',new.idproduct,'2022-11-19','UT',1,'admin','13:22:00','STOK AWAL');
*/
END;
...
AUTO_INCREMENT=60 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci'
COMMENT='InnoDB free: 731136 kB';
...
AUTO_INCREMENT=11218 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci'
COMMENT='InnoDB free: 731136 kB';
...
CREATE TABLE `customerdiskon` (
  ...
  CONSTRAINT `customerdiskon_fk1` FOREIGN KEY (`merk`) REFERENCES `product` (`merk`)
)ENGINE=InnoDB
AUTO_INCREMENT=19172 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci';
...
AUTO_INCREMENT=7700 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci'
COMMENT='InnoDB free: 731136 kB';
...
BEGIN
 ...
 where month(tanggal)=month(new.tanggal) and year(tanggal)=year(new.tanggal);
...
CREATE DEFINER = 'metabrain'@'%' TRIGGER `sellmaster_before_upd_tr` BEFORE UPDATE ON `sellmaster`
  FOR EACH ROW
BEGIN
 declare vquantity,viduser int(11);
 ...
 begin
  select coalesce(sum(subtotal),0) into ttlharga from selldetail  
...
CREATE DEFINER = 'metabrain'@'%' TRIGGER `sellorder_before_del_tr` BEFORE DELETE ON `sellorder`
  FOR EACH ROW
BEGIN
  insert into sellmaster (faktur,idsellorder,tanggal,waktu,kasir,IDcustomer,kodecustomer,customer,alamat,kota,pembayaran,nogiro,tgljatuhtempo,subtotal,ppn,total,diskon,diskonrp,grandtotal,bayar,kembali,isposted,lunas,totalpayment,kodesales,namasales,bayarskrg,kodepromo,checked) 
  select DISTINCT(d.kategori),old.idsell,old.tanggal,old.waktu,old.kasir,old.IDcustomer,old.kodecustomer,old.customer,old.alamat,old.kota,old.pembayaran,old.nogiro,old.tgljatuhtempo,old.subtotal,old.ppn,old.total,old.diskon,old.diskonrp,old.grandtotal,old.bayar,old.kembali,0,old.lunas,old.totalpayment,old.kodesales,old.namasales,old.bayarskrg,old.kodepromo,old.checked from sellorderdetail d 
  where d.idsell=old.idsell;
 
  insert into selldetail (faktur,kode,nama,kategori,merk,tipe,seri,hargajual,diskon,diskon2,diskonrp,diskon_rp,quantity,hargabeli,satuan,subtotal,isposted,idstts,idsell,komisisales,kodepromo) 
  select j.faktur,d.kode,d.nama,d.kategori,d.merk,d.tipe,d.seri,d.hargajual,d.diskon,d.diskon2,d.diskonrp,d.diskon_rp,d.quantity,d.hargabeli,d.satuan,d.subtotal,d.isposted,d.idstts,j.idsell,d.komisisales,d.kodepromo from `sellorderdetail` d 
...
AUTO_INCREMENT=344 CHARACTER SET 'latin1' COLLATE 'latin1_swedish_ci'
COMMENT='InnoDB free: 731136 kB';
