...
type
  TfrmSrcProd = class(TForm)
    ...
    pnlFreeTukar: TRzPanel;
    SearchccxFree: TRzCheckBox;
    SearchccxTukar: TRzCheckBox;
    ...
    procedure SearchccxFreeClick(Sender: TObject);
    procedure SearchccxTukarClick(Sender: TObject);
  private
...
uses Data, SparePartFunction, frmSelling, frmSellingOrder, frmbuying, frmAdjust, pindahgudang, promodiskon, ubahharga;
...
//  KodeGdg := kodegudang;
  ...
  SearchccxFree.Checked := false;
  SearchccxTukar.Checked := false;

  pnlFreeTukar.Visible := (formSender=frmSell)or(formSender=frmSellOrder);

  ccx_isreorder.Checked :=false;
  ccx_isreorder.Visible :=false;

{  if formSender=frmRtrJual then
  begin
   pnlfakturjual.Visible:=true;    }
//   FillComboBox('faktur','sellmaster where kodecustomer = '+QuotedStr(frmRtrJual.cbx_kodecust.text)+' and isposted = 1 ',cbx_fakturjual,false,'',True);
...
//    if formSender=frmRtrJual then
//    begin
//      SQL.Strings[0] := 'select cast(a.quantity as decimal) qty,a.kode,a.nama,a.merk,a.seri,a.satuan,cast("" as char(50)) supplier,a.kategori,a.hargabeli,a.hargajual'+',cast("" as char(50)) keterangan,cast(null as date) tglnoneffective,a.diskon,a.diskonrp,a.kodepromo,a.faktur from selldetail a ';
{      SQL.Strings[0] := 'select a.quantity qty,a.kode,a.nama,a.merk,a.seri,a.satuan,a.kategori,a.hargajual'+',cast("" as char(50)) keterangan,cast(null as date) tglnoneffective,a.diskon,a.diskonrp,a.faktur,a.komisi_ms,a.komisi_ex from selldetail a ';
      ...
      begin
       SQL.Strings[1] := 'left join sellmaster s on a.faktur = s.faktur where s.kodecustomer = '+ QuotedStr(frmRtrJual.cbx_kodecust.text) +' and s.isposted = 1 and a.' + SearchCategories + ' like ' + QuotedStr(SearchTxtSearch.Text + '%') + ' ';
      end;  }
//    end
//    else
//    begin
//      SQL.Strings[0] := 'select a.qty,b.*,cast("" as char(20)) faktur from v_stock a left join product b on a.kodebrg = b.kode';
//      SQL.Strings[1] := 'where a.kodegudang = '+ QuotedStr(KodeGdg) +' and b.' + SearchCategories + ' like ' + QuotedStr(SearchTxtSearch.Text + '%') + ' '; //and a.qty <> 0';
       SQL.Strings[0] := 'select p.*,cast("" as char(20)) faktur from product p ';
...
{      SQL.Strings[0] := 'select a.quantity qty,a.kode,a.nama,a.merk,a.seri,a.satuan,a.kategori,a.hargajual'+',cast("" as char(50)) keterangan,cast(null as date) tglnoneffective,a.diskon,a.diskonrp,a.faktur,a.komisi_ms,a.komisi_ex from selldetail a ';
      ...
      begin
       SQL.Strings[1] := 'left join sellmaster s on a.faktur = s.faktur where s.kodecustomer = '+QuotedStr(frmRtrJual.cbx_kodecust.text)+' and s.isposted = 1 ';
      end;      }
//    end
//    else
//    begin
...
begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select * from formsell');
      SQL.Add('where kode = ' + QuotedStr(DataModule1.ZQrySearchProductkode.Value) + ' and ipv=' + Quotedstr(ipcomp) );
      Open;
      if (not IsEmpty) then Exit;
    end;
  //  if DataModule1.ZQryFormSell.State <> dsEdit then
    ...
    DataModule1.ZQryFormSelltipe.Value := DataModule1.ZQrySearchProducttipe.Value;
    ...
    DataModule1.ZQryFormSelldiskon.Value := getdatanum('disc','customerdiskon where IDcustomer='+inttostr(frmSell.vidcust)+' and merk='+Quotedstr(DataModule1.ZQrySearchProductmerk.Value));
    DataModule1.ZQryFormSelldiskon2.Value := getdatanum('diskon','product where kode='+ Quotedstr(DataModule1.ZQrySearchProductkode.Value) );

//    DataModule1.ZQryFormSelldiskonrp.Value := round((DataModule1.ZQryFormSelldiskon.Value * DataModule1.ZQrySearchProducthargajual.Value) / 100);
    DataModule1.ZQryFormSelldiskonrp.Value := round((DataModule1.ZQrySearchProducthargajual.Value * DataModule1.ZQryFormSelldiskon.Value * 0.01)+((DataModule1.ZQrySearchProducthargajual.Value - (DataModule1.ZQrySearchProducthargajual.Value * DataModule1.ZQryFormSelldiskon.Value * 0.01)) * DataModule1.ZQryFormSelldiskon2.Value * 0.01));

    if SearchccxFree.Checked then
    begin
     DataModule1.ZQryFormSellidstts.Value := 1;
     DataModule1.ZQryFormSellvsubtotal.Value := 'FREE';
    end
    else if SearchccxTukar.Checked then
    begin
     DataModule1.ZQryFormSellidstts.Value := 2;
     DataModule1.ZQryFormSellvsubtotal.Value := 'TUKAR';
    end
    else
    begin
     DataModule1.ZQryFormSellidstts.Value    := 0;
     DataModule1.ZQryFormSellvsubtotal.Value := '0';
    end;

    DataModule1.ZQryFormSellipv.Value    := ipcomp;

    DataModule1.ZQryFormSellnomordet.value := DataModule1.ZQryFormSell.Recordcount+1;
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select count(*) juml,kategori from formsellorder');
      SQL.Add('where ipv=' + Quotedstr(ipcomp) + ' group by kategori having juml>=90 ');
      Open;
      while not eof do
      begin
       if (DataModule1.ZQrySearchProductkategori.Value=Fields[1].AsString) then
       begin
        ErrorDialog('Barang Divisi '+ Fields[1].AsString +' tidak boleh lebih dari 90 baris (Jenis Barang) !');
        Exit;
       end;
       next;
      end;
    end;
  //  if DataModule1.ZQryFormSellOrder.State <> dsEdit then
    DataModule1.ZQryFormSellOrder.Append;
    DataModule1.ZQryFormSellOrderfaktur.Value := frmSellOrder.SellTxtNo.Text;
    DataModule1.ZQryFormSellOrderkode.Value := DataModule1.ZQrySearchProductkode.Value;
    DataModule1.ZQryFormSellOrdernama.Value := DataModule1.ZQrySearchProductnama.Value;
    DataModule1.ZQryFormSellOrderharga.Value := DataModule1.ZQrySearchProducthargajual.Value;
    DataModule1.ZQryFormSellOrderkategori.Value := DataModule1.ZQrySearchProductkategori.Value;
    DataModule1.ZQryFormSellOrdermerk.Value := DataModule1.ZQrySearchProductmerk.Value;
    DataModule1.ZQryFormSellOrdertipe.Value := DataModule1.ZQrySearchProducttipe.Value;
    DataModule1.ZQryFormSellOrderseri.Value := DataModule1.ZQrySearchProductseri.Value;
    DataModule1.ZQryFormSellOrdersatuan.Value := DataModule1.ZQrySearchProductsatuan.Value;

    DataModule1.ZQryFormSellOrderdiskon.Value := getdatanum('disc','customerdiskon where IDcustomer='+inttostr(frmSellOrder.vidcust)+' and merk='+Quotedstr(DataModule1.ZQrySearchProductmerk.Value));
    DataModule1.ZQryFormSellOrderdiskon2.Value := getdatanum('diskon','product where kode='+ Quotedstr(DataModule1.ZQrySearchProductkode.Value) );

  //  DataModule1.ZQryFormSellOrderdiskonrp.Value := round((DataModule1.ZQryFormSellOrderdiskon.Value * DataModule1.ZQrySearchProducthargajual.Value) / 100);

    DataModule1.ZQryFormSellOrderdiskonrp.Value := round((DataModule1.ZQrySearchProducthargajual.Value * DataModule1.ZQryFormSellOrderdiskon.Value * 0.01)+((DataModule1.ZQrySearchProducthargajual.Value - (DataModule1.ZQrySearchProducthargajual.Value * DataModule1.ZQryFormSellOrderdiskon.Value * 0.01)) * DataModule1.ZQryFormSellOrderdiskon2.Value * 0.01));

    DataModule1.ZQryFormSellOrderhargabeli.Value := DataModule1.ZQrySearchProducthargabeli.Value;

    if SearchccxFree.Checked then
    begin
     DataModule1.ZQryFormSellOrderidstts.Value := 1;
     DataModule1.ZQryFormSellOrdervsubtotal.Value := 'FREE';
    end
    else if SearchccxTukar.Checked then
    begin
     DataModule1.ZQryFormSellOrderidstts.Value := 2;
     DataModule1.ZQryFormSellOrdervsubtotal.Value := 'TUKAR';
    end
    else
    begin
     DataModule1.ZQryFormSellOrderidstts.Value    := 0;
     DataModule1.ZQryFormSellOrdervsubtotal.Value := '0';
    end;

    DataModule1.ZQryFormSellOrderipv.Value    := ipcomp;

    DataModule1.ZQryFormSellOrdernomordet.value := DataModule1.ZQryFormSellOrder.Recordcount+1;

    DataModule1.ZQryFormSellOrder.CommitUpdates;
    SearchBtnCloseClick(Self);

    frmSellOrder.PenjualanDBGrid.SetFocus;
    frmSellOrder.PenjualanDBGrid.SelectedIndex := 4;
  end
{  else
  if formSender = frmRtrJual then
  begin
   ...
   {   Close;
      SQL.Clear;
      SQL.Add('select * from formretur where ipv='+Quotedstr(ipcomp));
...
{      qtytamp:=0;
      Close;
      SQL.Clear;
      SQL.Add('select getQtysisaReturJual('+ QuotedStr(DataModule1.ZQrySearchProductfaktur.Value) +','+ QuotedStr(DataModule1.ZQrySearchProductkode.Value) +') ');
      Open;
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select kode from formbuy ');
 ...
 //   if DataModule1.ZQryFormBuy.State <> dsEdit then
    ...
    DataModule1.ZQryFormbuytipe.Value := DataModule1.ZQrySearchProducttipe.Value;
    ...
    frmbuy.BuyDBGrid.SelectedIndex := 4;
  end
  else
  ...
  begin
    ...
    DataModule1.ZQryFormAdjusttipe.Value := DataModule1.ZQrySearchProducttipe.Value;

    case frmAdj.AdjustTxtGudang.ItemIndex of
      0 : DataModule1.ZQryFormAdjustquantity.Value := DataModule1.ZQrySearchProductUT.Value;
      1 : DataModule1.ZQryFormAdjustquantity.Value := DataModule1.ZQrySearchProductRS.Value;
    end;
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select * from pindahgudangdetform ');
      SQL.Add('where ipv='+Quotedstr(ipcomp)+' and kode = ' + QuotedStr(DataModule1.ZQrySearchProductkode.Value) + ' ');
      Open;
      if (not IsEmpty) then
      begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select * from ubahhargadetform ');
      SQL.Add('where ipv='+Quotedstr(ipcomp)+' and kode = ' + QuotedStr(DataModule1.ZQrySearchProductkode.Value) + ' ');
      Open;
      if (not IsEmpty) then
      begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('select * from diskondetform ');
      SQL.Add('where ipv='+Quotedstr(ipcomp)+' and kode = ' + QuotedStr(DataModule1.ZQrySearchProductkode.Value) + ' ');
      Open;
      if (not IsEmpty) then
      begin
...
procedure TfrmSrcProd.SearchccxFreeClick(Sender: TObject);
begin
 if SearchccxFree.Checked then SearchccxTukar.Checked := false;
end;

procedure TfrmSrcProd.SearchccxTukarClick(Sender: TObject);
begin
 if SearchccxTukar.Checked then Searchccxfree.Checked := false;
end;
