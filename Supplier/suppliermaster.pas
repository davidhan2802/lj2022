...
procedure TFrmsuppliermaster.AdvSmoothButton1Click(Sender: TObject);
begin
  if (DataModule1.ZQrySupplier.IsEmpty)or(DataModule1.ZQrySupplierkode.IsNull) then
  begin
   ErrorDialog('Belum ada Supplier !');
   exit;
  end;

//  if QuestionDialog('Print Amplop Untuk Supplier ini?') = False then Exit;

  PrintAmplopSupp(DataModule1.ZQrySupplierkode.Value);
end;
