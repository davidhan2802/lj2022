...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select g.IDgolongan,g.kode,g.nama,sum(i.qty) qty, sum(i.qty)*p.hargabeli value from inventory i ');
    SQL.Add('left join product p on i.kodebrg=p.kode ');
    SQL.Add('left join golongan g on p.IDgolongan=g.IDgolongan ');
    SQL.Add('where '+strgdg+'(i.tgltrans between ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchFirst.Date) + ''' ');
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select p.IDgolongan,p.kode,p.barcode,p.nama,sum(i.qty) qty, sum(i.qty)*p.hargabeli value from inventory i ');
    SQL.Add('left join product p on i.kodebrg=p.kode ');
    SQL.Add('where '+strgdg+'(i.tgltrans between ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchFirst.Date) + ''' ');
    SQL.Add('and ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchLast.Date) + ''') and (p.IDgolongan=:IDgolongan) ');
    SQL.Add('group by p.IDgolongan,p.kode,p.nama having qty<>0 ');
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select sum(i.qty)*p.hargabeli value from inventory i ');
    SQL.Add('left join product p on i.kodebrg=p.kode ');
    SQL.Add('where '+strgdg+'(i.tgltrans between ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchFirst.Date) + ''' ');
    SQL.Add('and ''' + FormatDateTime('yyyy-MM-dd',SellTxtSearchLast.Date) + ''') ');
    Open;
