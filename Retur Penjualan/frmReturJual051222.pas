...
type
  TfrmRtrJual = class(TForm)
    ...
    procedure RtrJualTxtcustomerKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses SparePartFunction, frmSearchProduct, frmSearchCust, Data;
...
begin
  ...
  if RtrTxtGudang.ItemIndex=0 then vkodegdg:='UT'
  else if RtrTxtGudang.ItemIndex=1 then vkodegdg:='RS';
  ...
  begin
    ...
    SQL.Add('select '+Quotedstr(idTrans)+',faktur,kode,nama,kategori,merk,seri,keterangan,quantity,quantityretur,hargabeli,harga,diskon,diskon2,diskon_rp,diskonrp,satuan,fakturjual,'+QuotedStr(vkodegdg)+',subtotal ');
...
procedure TfrmRtrJual.RtrJualTxtcustomerKeyDown(Sender: TObject;
...
//    vKodeCust := FieldByName('kodecustomer').AsString;
 ...
 //   RtrTxtpembayaran.ItemIndex := RtrTxtpembayaran.Items.IndexOf(FieldByName('pembayaran').AsString);
    ...
    SQL.Add('select '+Quotedstr(ipcomp)+',d.kode,d.nama,getQtysisaReturJual(d.faktur,d.kode),d.satuan,d.hargajual,'+Quotedstr(vkodegdg)+',d.kategori,d.merk,d.tipe,d.seri,"'+RtrJualTxtFaktur.Text+'",d.faktur,d.diskon,d.diskon2,d.diskon_rp,d.hargabeli from selldetail d ');
